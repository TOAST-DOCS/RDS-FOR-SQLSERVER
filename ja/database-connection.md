...
## Database > RDS for MS-SQL > データベース接続 { #database-rds-for-ms-sql-database-connection }
...
## データベースポート { #database-port }

1150～49151の任意のポートをデータベースのポートに指定できます。
...
| リモート    | IPアドレスの範囲を指定できます。ルールの方向が「送信」の場合は宛先がリモートになり、「受信」の場合は送信元がリモートになります。 <br>ポリシーの方向に応じて、トラフィックの送信元または宛先がIPアドレスや範囲であるかを比較します。                                      |
