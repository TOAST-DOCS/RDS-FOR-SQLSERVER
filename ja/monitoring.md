...
## Database > RDS for MS-SQL > モニタリング { #database-rds-for-ms-sql-monitoring }
...
サーバーダッシュボードを通して、性能指標をチャート形式で視覚化して確認できます。指標は1分毎に収集され、最長1年間保管されます。各集計単位の保管期間は下記の通りです。
