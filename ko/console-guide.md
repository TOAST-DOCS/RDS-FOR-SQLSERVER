...
![DB 인스턴스 상세 > 오브젝트 스토리지에 백업](http://static.toastoven.net/prod_rds_mssql/20210209/output/db_instance_obs_backup_menu.png)

DB 인스턴스 목록에서 특정 DB 인스턴스를 선택 한 후, [오브젝트 스토리지로 백업] 버튼을 클릭하면 아래와 같은 팝업이 노출됩니다.

![DB 인스턴스 상세 > 오브젝트 스토리지에 백업 팝업](http://static.toastoven.net/prod_rds_mssql/20210209/output/db_instance_backup_to_obs_modal.png)

* ❶ 백업 파일이 저장될 오브젝트 스토리지의 Tenant ID를 입력합니다. 오브젝트 스토리지 서비스 웹 콘솔의 API 엔드포인트 설정에서 확인할 수 있습니다.
* ❷ 백업 파일이 저장될 오브젝트 스토리지의 TOAST 계정(이메일)을 입력합니다.
...
정보를 입력한 후, [확인] 버튼을 누르면 백업이 진행됩니다. 백업이 완료되면 입력한 오브젝트 스토리지의 컨테이너에서 백업 파일 다운로드 받을 수 있습니다.

![오브젝트 스토리지에 올려진 백업 파일 확인](http://static.toastoven.net/prod_rds_mssql/20210209/output/db_instance_backup_to_obs_result.png)

### 오브젝트 스토리지에 있는 백업으로 복원

오브젝트 스토리지에 있는 백업 파일을 DB 인스턴스에 복원 할 수 있습니다.

![DB 인스턴스 상세 > 오브젝트 스토리지에 있는 백업으로 복원](http://static.toastoven.net/prod_rds_mssql/20210209/output/db_instance_obs_backup_menu.png)

DB 인스턴스 목록에서 특정 DB 인스턴스를 선택 한 후, [오브젝트 스토리지에 있는 백업으로 복원] 버튼을 클릭하면 아래와 같은 팝업이 노출됩니다.

![DB 인스턴스 상세 > 오브젝트 스토리지에 있는 백업으로 복원 팝업](http://static.toastoven.net/prod_rds_mssql/20210209/output/db_instance_backup_from_obs_modal.png)
...
![백업 목록 002](http://static.toastoven.net/prod_rds_mssql/20210209/output/backup_to_obs_menu.png)

백업 목록에서 오브젝트 스토리지로 내보낼 백업을 선택한 후, [오브젝트 스토리지로 백업 내보내기] 버튼을 클릭합니다.

![백업 목록 > 오브젝트 스토리지로 백업 내보내기 팝업](http://static.toastoven.net/prod_rds_mssql/20210209/output/backup_to_obs_modal.png)
