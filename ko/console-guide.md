...
* ❽ 장애 조치 혹은 로그 백업 유실로 시점 복원이 불가능할 때 버튼이 노출됩니다. 버턴을 클릭해서 자동 백업을 생성하고 시점 복원이 가능하게 만들 수 있습니다.
...
![DB 인스턴스 상세 > 오브젝트 스토리지에 백업](https://static.toastoven.net/prod_rds_mssql/20230110/db_instance_obs_backup_menu.png)

DB 인스턴스 목록에서 특정 DB 인스턴스를 선택 한 후, **오브젝트 스토리지로 차등 백업** 버튼을 클릭하면 아래와 같은 팝업이 노출됩니다.

![DB 인스턴스 상세 > 오브젝트 스토리지에 백업 팝업](https://static.toastoven.net/prod_rds_mssql/20230110/db_instance_differential_backup_to_obs_backup.png)
...
* ❶ 전체 백업 파일이 저장되어있는 오브젝트 스토리지의 Tenant ID를 입력합니다. 오브젝트 스토리지 서비스 웹 콘솔의 API 엔드포인트 설정에서 확인할 수 있습니다.
* ❷ 전체 백업 파일이 저장되어있는 오브젝트 스토리지의 NHN Cloud 계정(이메일)을 입력합니다.
* ❸ 전체 백업 파일이 저장되어있는 오브젝트 스토리지의 API 비밀번호를 입력합니다.
* ❹ 전체 백업 파일이 저장되어있는 오브젝트 스토리지의 컨테이너 이름을 입력합니다.
* ❺ 컨테이너에 저장되어있는 전체 백업 파일의 경로를 입력합니다.
    * 폴더 이름은 최대 255바이트이고, 전체 경로는 최대 1024바이트입니다.
    * 특정 형태(. 또는 ..)는 사용할 수 없으며 특수문자(' " < > ; /)와 공백은 입력할 수 없습니다.
* ❻ 복원될 데이터베이스의 이름을 입력합니다.
* ❼ 차등 백업 복원 여부를 선택합니다.
* ❽ 컨테이너에 저장되어있는 차등 백업 파일의 경로를 입력합니다.
